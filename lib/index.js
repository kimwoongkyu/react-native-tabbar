...
export default class Tabar extends Component {
  ...
  makeState(height, offset, step, show) {
    ...
    return {
      ...
      bottom: new Animated.Value(show ? 0 : -height),
      scroll: magicScroll(0, height, offset, step)
    }
  }

  updateHeight = (scrollY, direction) => {
    let height
    if (!this.props.disable) {
      height = this.state.scroll.calc(scrollY)
      if('down' === direction && this.prevDirection !== direction){
        this.prevDirection = direction
        this.hide()
      } else if ('up' === direction && this.prevDirection !== direction) {
        this.prevDirection = direction
        this.show()
      }
  ...
  get window() {
    const offsetHeight = Platform.OS === 'android' ? 72 : 72
    const window = {
      height: Dimensions.get('window').height - offsetHeight,
  ...
  recalculate = () => {
    ...
    if (this.props.disable) {
      ...
      this.state.bottom.setValue(this.props.height)
    }
  }
